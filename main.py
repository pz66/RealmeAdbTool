...
class Window:
    def __init__(self, root):
        self.root = root
        self.root.title('Realme adb Tool v1.0')
    ...
    def initUI(self):
        ...
        self.secondAppList = tk.Listbox(frame_secondApp,yscrollcommand=secondAppListBar.set,relief='flat',bg='lightgrey')
        ...
        frame_install_2 = tk.LabelFrame(self.frame_install,text='传送文件到手机Download目录（请拖放文件到此处），不支持中文')
        ...
        ttk.Button(frame_install_1,text='安装',command=self.installApps).grid(row=0,column=1,rowspan=1,columnspan=1,padx=10)
        ttk.Button(frame_install_1,text='降级安装',command=self.installAppsWithData).grid(row=1,column=1,rowspan=1,columnspan=1,padx=10)
        ttk.Button(frame_install_2,text='传送',command=self.transferFiles).grid(row=0,column=1,rowspan=1,columnspan=1,padx=10)

        self.frame_restart = tk.Frame()
        ttk.Button(self.frame_restart,text='重启手机',command=self.reboot).grid(row=0,column=0,padx=30,pady=10,sticky='we')
        ttk.Button(self.frame_restart,text='关机',command=self.shutdown).grid(row=1,column=0,padx=30,pady=10,sticky='we')
        ttk.Button(self.frame_restart,text='重启到Recovery',command=self.rebootRecovery).grid(row=2,column=0,padx=30,pady=10,sticky='we')
        ttk.Button(self.frame_restart,text='重启到Bootloader',command=self.rebootBootloader).grid(row=3,column=0,padx=30,pady=10,sticky='we')
